...
- **Row 1 增第三個 FilterGroup「醫師」**（跟「狀態」「品牌」並列）
- chip 列：「全部」+ 每位有病患的醫師、按病患數降序排
- 點某醫師 → 列表只剩該醫師的病患
- 顯示每位醫師的病患數（標在 chip 右上）
- 沒有任何醫師資料時、整個 group 隱藏（不佔位）
- 「重置篩選」按鈕同步清掉醫師 filter

### 為什麼

「有醫師想要看自己的病患」— 不用每次都打姓名搜尋、不用為每位醫師建獨立帳號。
純前端 filter，client side 即時切換。

### 注意

這跟「警示」區的「⚠ 未登記醫師」是兩件事：
- 醫師 filter：選定某醫師、只看他的（已登錄那批）
- 警示：標出沒登錄醫師的病患（doctor 欄為空）— 待補資料
