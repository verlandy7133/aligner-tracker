// 病患筆記 + 8-slot 病歷照片 整合 component
//
// 結構：
//   ┌────────────────────────────────────────────┐
//   │ 📋 病患筆記 / 照片                          │
//   ├────────────────────────────────────────────┤
//   │ [markdown 編輯區 — auto-save 500ms debounce]│
//   │                                              │
//   │ 8-slot photo grid:                          │
//   │  [Pano X-ray]  [Ceph X-ray]                │
//   │  [Front Cls]  [Front Opn]                  │
//   │  [Left Cls]   [Right Cls]                  │
//   │  [Upper Occ]  [Lower Occ]                  │
//   └────────────────────────────────────────────┘
...
import { db } from '../db';
import type { Patient, PhotoSlot } from '../types/Patient';
import { PHOTO_SLOTS } from '../types/Patient';
  ...
  return (
    <section className="rounded-xl border border-zinc-800 bg-zinc-900/30">
      <header className="px-5 py-3 border-b border-zinc-800">
        <h2 className="text-sm font-medium text-zinc-200">📋 病患筆記 / 病歷照片</h2>
      </header>
    ...
    <div>
      <h3 className="text-xs uppercase tracking-wider text-zinc-500 font-medium mb-2">
        ...
        <span className="ml-2 text-[10px] text-zinc-600 normal-case tracking-normal">
          照片來源：<code>{patient.sourceFolder}</code>
        </span>
      </h3>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {PHOTO_SLOTS.map((slot) => (
          <PhotoSlotCell
            ...
            filename={patient.photos?.[slot.key]}
...
function PhotoSlotCell({
  ...
  const fullPath = filename ? `${patient.sourceFolder}\\${filename}` : null;
  ...
  return (
    <div className="relative aspect-[4/3] rounded-md overflow-hidden border border-zinc-800 bg-zinc-950 group">
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent px-2 py-1">
        ...
        <div className="text-[9px] text-white/60 truncate font-mono">{filename}</div>
      </div>
          ...
          {files && files.length > 0 && (
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
              {files.map((f) => {
                ...
                const isCurrent = patient.photos?.[slot] === f;
                return (
                  <button
                    key={f}
                    onClick={() => pick(f)}
